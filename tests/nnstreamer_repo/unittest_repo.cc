...
TEST (tensorRepo, removeWakesAllWaiters)
{
  ...
  wait_ctx_init (&job1.ctx);
  job1.op = OP_SET_BUFFER;
  job1.slot = slot;
  job1.in_buffer = buf1;
  job1.in_caps = caps;

  wait_ctx_init (&job2.ctx);
  job2.op = OP_SET_BUFFER;
  job2.slot = slot;
  job2.in_buffer = buf2;
  job2.in_caps = caps;
  ...
  wait_ctx_clear (&job1.ctx);
  wait_ctx_clear (&job2.ctx);
...
TEST (tensorRepo, pipelineSinkErrorsWhenSlotRemoved)
{
  ...
  /* nobody pulls from the slot: the sink blocks pushing its second buffer */
  g_usleep (300 * 1000);
